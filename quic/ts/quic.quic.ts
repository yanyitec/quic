...
namespace Quic{
    ...
    export class QuicModule implements IQuic{
        /**
         * 模块名
         * 
         * @type {string}
        ...
        ready(handler:(module:IQuic)=>void){
        ...
        init(opts:QuicOpts){
            ...
            this.ready = (handler:(module:IQuic)=>void):void=>{
                handler(this);
            }
        }
        /**
    ...
    export let cached_modules :{[name:string]:IQuic}={};
    let loadQuic=(name:string ,comlete:(module:IQuic)=>void)=>{
        let module = cached_modules[name];
        if(!module) module = cached_modules[name]=new QuicModule();
