...
(function (Quic) {
    var head;
    var loadScript = Quic.env.loadScript = function (url, extras) {
        if (!head) {
            var heads = Quic.dom.getElementsByTagName("head");
            head = heads[0];
        }
        var result = new Quic.Promise(function (resolve, reject) {
            var element = Quic.dom.createElement("script");
    ...
    var loadCss = Quic.env.loadCss = function (url, extras) {
        if (!head) {
            var heads = Quic.dom.getElementsByTagName("head");
            head = heads[0];
        }
        var result = new Quic.Promise(function (resolve, reject) {
            var element = Quic.dom.createElement("link");
    ...
    Quic.env.loadContent = function (url, extras) { return Quic.ajax({
        url: url,
        method: "get"
    }); };
