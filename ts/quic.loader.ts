...
namespace Quic{
    ...
    let loadScript = env.loadScript =(url:string,extras?:any):IPromise=>{
        if(!head){
            let heads= dom.getElementsByTagName("head");
            head = <HTMLHeadElement>heads[0];
        }
        let result:IPromise = new Promise((resolve,reject)=>{
            let element:HTMLScriptElement = <HTMLScriptElement>dom.createElement("script");
    ...
    let loadCss = env.loadCss =(url:string,extras?:any):IPromise=>{
        if(!head){
            let heads= dom.getElementsByTagName("head");
            head = <HTMLHeadElement>heads[0];
        }
        let result:IPromise = new Promise((resolve,reject)=>{
            let element:HTMLLinkElement = <HTMLLinkElement>dom.createElement("link");
    ...
    env.loadContent =(url:string,extras?:any):IPromise=>ajax({
        url:url,
        method:"get"
    });
